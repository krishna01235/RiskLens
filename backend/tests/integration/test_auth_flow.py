...
@pytest.mark.asyncio
async def test_register_success(async_client: httpx.AsyncClient) -> None:
...
async def test_register_duplicate_email(async_client: httpx.AsyncClient) -> None:
    ...
    email = _unique_email()
    await async_client.post("/auth/register", json={"email": email, "password": "password123"})
    resp = await async_client.post(
...
@pytest.mark.asyncio
async def test_login_success(async_client: httpx.AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_login_wrong_password(async_client: httpx.AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_login_unknown_email(async_client: httpx.AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_refresh_success(async_client: httpx.AsyncClient) -> None:
    ...
    import asyncio
    await asyncio.sleep(1.1)  # Ensure 'exp' claim is at least 1 second different
...
@pytest.mark.asyncio
async def test_refresh_after_logout(async_client: httpx.AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_protected_endpoint_no_token(async_client: httpx.AsyncClient) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_current_user(token=None, db=async_client)  # type: ignore[arg-type]
